...
import { simplifyDocument } from '@/lib/gemini';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await simplifyDocument(validation.sanitized!);
    return createSuccessResponse({ result });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to simplify document.';
    console.error('[API /simplify]', message);

...
import { analyzeClauses } from '@/lib/gemini';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await analyzeClauses(validation.sanitized!);
    return createSuccessResponse({ result });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to analyze clauses.';
